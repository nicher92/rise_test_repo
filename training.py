...
def train_epoch(train_loader, scheduler, optimizer, model, hyperparameter_search, i):
    ...
    for j, batch in enumerate(train_loader):
        _ids, at, lab = batch
        out = model(
            input_ids=_ids.to(model.device),
            attention_mask=at.to(model.device),
            labels=lab.to(model.device),
        )  #   logits = [256, 40, 31] B,T,C, loss is NLL
        ...
        if j % 25 == 0 and j > 0:
            ...
            if hyperparameter_search:
                ...
                )  # Loss needs to be 0 here otherwise error from tune
...
def train_model(
    params, model, training_dataset, evaluation_dataset, hyperparameter_search=True
):
    train_loader, eval_loader = ret_dataloaders(
        training_dataset, evaluation_dataset, params["batch_size"]
    )
    ...
    for i in range(1, params["epochs"] + 1):
        model, t_loss = train_epoch(
            train_loader, scheduler, optimizer, model, hyperparameter_search, i
        )
        e_loss = eval_epoch(eval_loader, model, hyperparameter_search, i)
        print("epoch: ", i)
        total_t_loss.extend(t_loss)
        total_e_loss.extend(e_loss)

    if not hyperparameter_search:
        return model, total_e_loss, total_t_loss
...
def random_hyperparameter_search(
    ...
    analysis = tune.run(
        ...
        config={
            ...
            "batch_size": tune.choice([16, 32]),  # Per original bert paper
            "epochs": tune.choice([2, 3, 4]),  # Per original bert paper
        },
    ...
    )  # Returns the configuration that gives the least loss on the evaluation set
